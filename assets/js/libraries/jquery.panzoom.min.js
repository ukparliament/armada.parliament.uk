	...
	Panzoom.prototype = {
		...
		_bind: function() {
			...
			if ($zoomRange.length) {
				...
				// 	console.log("zooming stuff");
				// };
				$zoomRange.on(events);

				// custom modification so that this works with the jquery UI slider
                $zoomRange[0].onInput = function(event) {
                    self.zoom(+this.value, { noSetRange: true });
                }
                $zoomRange[0].onchange = function(event) {
                    self.zoom(+this.value, { noSetRange: true });
                }
			}
		},
